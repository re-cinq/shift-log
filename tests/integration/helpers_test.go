...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Use Promise.race with a 5-second timeout: in some OpenCode versions the
      // client.session.messages() call returns a Promise that never resolves,
      // which would block the hook and prevent OpenCode from exiting.
      let transcriptData = "";
