## Context
VS Code's GitHub Copilot coding agent supports hooks via `.github/hooks/hooks.json` (must be on the default branch). The format differs from the Copilot CLI hook format (`claudit.json`), but the transcript format (`events.jsonl`) is identical. This means we only need to add hook file handling — the storage pipeline is fully reusable.
...
- **Separate hook file format**: VS Code uses `.github/hooks/hooks.json` with `bash`/`powershell` fields, not `claudit.json` with a `command` field. New read/write functions handle this.
...
|--------|-------------|---------------------|
| File | `.github/hooks/claudit.json` | `.github/hooks/hooks.json` |
...
func (a *Agent) ConfigureHooks(repoRoot string, vscode bool) error {
    ...
    // Else: CLI mode (.github/hooks/claudit.json)
}
```
...
```json
{
  ...
  "hooks": {
    "postToolUse": [{
      ...
      "powershell": "claudit store --agent=copilot",
      "cwd": ".",
      "timeoutSec": 30
    }],
    "sessionStart": [{
      "type": "command",
      "bash": "claudit session-start",
      "powershell": "claudit session-start",
      "timeoutSec": 10
    }],
    "sessionEnd": [{
      "type": "command",
      "bash": "claudit session-end",
      "powershell": "claudit session-end",
      "timeoutSec": 10
    }]
  }
}
...
## Risks / Trade-offs
- `.github/hooks/hooks.json` must be on the default branch — `claudit init --vscode` will need to warn about this
...
## Open Questions
- Should `claudit init --agent=copilot` auto-detect VS Code vs CLI, or always require `--vscode`?
