...
- [ ] 1.5 Implement `AddClauditVSCodeHooks()` to merge claudit hooks into existing file

## 2. Init Command Integration
- [ ] 2.1 Add `--vscode` flag to `claudit init` when `--agent=copilot`
...
## 3. Doctor Validation
- [ ] 3.1 Add VS Code hook file validation to `claudit doctor`
...
- [ ] 4.2 Acceptance tests for `claudit init --agent=copilot --vscode`
- [ ] 4.3 Acceptance tests for `claudit doctor` with VS Code hooks
- [ ] 4.4 Regression test: `claudit init --agent=copilot` still creates CLI format
