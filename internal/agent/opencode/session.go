...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Sanitize projectID (it's a git hash so safe, but be defensive)
	if strings.ContainsAny(projectID, "'\";\\") {
		return nil, nil
	}

	// Try multiple ordering strategies to handle different OpenCode schema versions
	...
	// Check if this session was recent (within timeout). Try multiple time column names.
	timeQueries := []string{
	...
	for _, q := range timeQueries {
		...
		if parsed {
			break // Found and checked a valid time column
		}
		// If time can't be parsed, proceed anyway — better to try than skip
		break
	}

	transcriptData := querySessionTranscript(dbPath, sessionID)
