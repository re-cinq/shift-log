...
//
// A 5-second timeout guards the client.session.messages() SDK call, which can
// hang indefinitely in some opencode versions (observed in v1.17+). On timeout
// the plugin falls back to the data_dir path and lets shiftlog read from SQLite.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Guard with a 5-second timeout: client.session.messages() can hang
      // indefinitely in some opencode versions (v1.17+), and if it does the
      // hook blocks opencode's exit. On timeout we fall back to the SQLite
      // path handled by 'shiftlog store' itself.
      let transcriptData = "";
      if (client && pending.sessionID) {
        try {
          ...
          // Fall back to data_dir/SQLite approach below
        }
      }
