...
//
// client.session.messages() is wrapped in Promise.race with a 5s timeout to
// prevent it from hanging indefinitely (observed in OpenCode v1.17+).
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API with a 5s timeout to
      // avoid hanging if the API is unresponsive (observed in OpenCode v1.17+).
      let transcriptData = "";
      if (client && pending.sessionID) {
        try {
          ...
          if (typeof client?.session?.messages === "function") {
            msgsPromise = client.session.messages({ path: { id: pending.sessionID } });
          } else if (typeof client?.message?.list === "function") {
            msgsPromise = client.message.list({ query: { sessionId: pending.sessionID } });
          }
          if (msgsPromise) {
