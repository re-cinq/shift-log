...
//
// On every tool.execute.after we write a session marker to
// <directory>/.shiftlog/opencode-session.json so the post-commit hook can
// discover the active session even when opencode itself did not make a commit.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      // Fetch current transcript once; reused for the marker and git-commit handler.
      let transcriptData = "";
      ...
      // Always write a session marker so the post-commit hook can discover this
      // session even when opencode did not itself make a git commit.
      if (input.sessionID) {
        try {
          const { mkdirSync, writeFileSync } = await import("fs");
          const { join } = await import("path");
          const slDir = join(directory, ".shiftlog");
          mkdirSync(slDir, { recursive: true });
          writeFileSync(join(slDir, "opencode-session.json"), JSON.stringify({
            sessionId: input.sessionID,
            dataDir: dataDir,
            timestamp: Date.now(),
          ...
          // Silently ignore — marker is best-effort
        }
      }

      // Handle git commits: call shiftlog store directly.
