...
// On every tool call we also write .shiftlog/opencode-session.json so that the
// git post-commit hook can find the active session without needing sqlite3.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Record active session so the git post-commit hook can find it
