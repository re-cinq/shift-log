...
//
// Compatibility notes for opencode v1.15+:
//   - input.sessionID may be undefined in some versions; fall back to input.session_id
//   - messages may use 'parts' (array) instead of 'content' (string)
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (command.includes("git commit") || command.includes("git-commit")) {
        // Support both sessionID (pre-v1.15) and session_id (v1.15+) field names
        ...
        try {
          ...
          if (msgs && Array.isArray(msgs)) {
            transcriptData = JSON.stringify(msgs.map(m => ({
              role: m.role || "",
              id: m.id || "",
              // Include both content (pre-v1.15) and parts (v1.15+) formats
              ...(m.parts !== undefined
                ? { parts: m.parts }
                : { content: m.content || "" }),
              time: m.time || {},
