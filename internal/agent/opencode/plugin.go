...
//
// client.session.messages() is called with a 5-second timeout to prevent the hook
// from hanging if the API endpoint blocks (observed in OpenCode v1.16+).
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API with a short timeout to
      // avoid blocking opencode if the endpoint hangs (e.g. OpenCode v1.16+).
      let transcriptData = "";
      if (client && pending.sessionID) {
        try {
          const msgPromise = client.session.messages({ path: { id: pending.sessionID } });
          const timeoutPromise = new Promise((_, reject) =>
            setTimeout(() => reject(new Error("timeout")), 5000));
          const msgs = await Promise.race([msgPromise, timeoutPromise]);
