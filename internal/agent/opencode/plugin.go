...
//
// We also write .shiftlog/opencode-session.json after every tool execution so
// that the post-commit git hook can discover the session for manual commits.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  const writeSessionState = async (sessionID) => {
    ...
    try {
      ...
      const state = JSON.stringify({
        ...
        transcript: JSON.stringify(msgs.map(m => ({
          role: m.role || "",
          id: m.id || "",
          content: m.content || "",
          time: m.time || {},
      ...
      writeFileSync(shiftlogDir + "/opencode-session.json", state);
    } catch (e) {}
  };
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      // Always write session state for manual commit discovery
      await writeSessionState(input.sessionID);
