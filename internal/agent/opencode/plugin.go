...
//
// The plugin also writes .shiftlog/opencode-session.json on each tool call so
// that the post-commit git hook can discover the active session for manual commits.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Update session tracking file on each tool call.
      // The post-commit git hook reads this to find the session for manual commits.
      if (input.sessionID) {
        try {
          fs.mkdirSync(shiftlogDir, { recursive: true });
          fs.writeFileSync(
            join(shiftlogDir, "opencode-session.json"),
            JSON.stringify({ session_id: input.sessionID, project_dir: directory }),
            "utf8"
          );
        } catch (e) {}
      }
