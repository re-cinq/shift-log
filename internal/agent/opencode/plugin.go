...
// We also write a session marker file on every before-hook invocation so that
// shiftlog can discover the session ID for manual commits via DiscoverSession.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Write session marker so shiftlog can discover this session for manual commits
      try {
        const { writeFileSync, mkdirSync } = await import("fs");
        const markerDir = directory + "/.shiftlog";
        mkdirSync(markerDir, { recursive: true });
        writeFileSync(
          markerDir + "/opencode-session.json",
          JSON.stringify({
