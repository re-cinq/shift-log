...
//
// On every tool execution we also write .git/shiftlog-session so that the
// post-commit hook can discover the session ID without depending on how
// opencode formats its internal project_id in the database.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Write session marker so the post-commit hook can discover this session
      // without depending on opencode's internal project_id storage format.
      if (input.sessionID) {
        try {
          const { writeFileSync } = await import("fs");
          writeFileSync(directory + "/.git/shiftlog-session", JSON.stringify({
