...
//
// On every tool.execute.after we also write .shiftlog/opencode-session.json so
// that shiftlog store --manual (the post-commit git hook) can discover the active
// session even when the user commits manually rather than via the agent. This is
// schema-independent: it does not rely on SQLite queries and works regardless of
// which version of opencode is installed.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      // Fetch transcript once; reused for both session state and commit hook.
      const transcriptData = await fetchTranscript(input.sessionID);

      // Always persist session state so shiftlog store --manual can find the
      // session after any tool execution, even when no git commit was made.
      if (input.sessionID && directory) {
        try {
          const fs = await import("fs");
          const shiftlogDir = directory + "/.shiftlog";
          fs.mkdirSync(shiftlogDir, { recursive: true });
          fs.writeFileSync(shiftlogDir + "/opencode-session.json", JSON.stringify({
          ...
          // Silently ignore — session file is best-effort
        }
      }

      // Handle git commits: call shiftlog store for the agent-triggered commit.
