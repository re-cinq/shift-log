...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API with a hard timeout.
      // In opencode v1.17+ the client.session.messages() promise may never
      // settle if the API shape changed; Promise.race ensures we don't block.
      let transcriptData = "";
