...
// On first tool execution we also write a session marker file so that
// "shiftlog store --manual" can discover the session even if the SQLite
// project_id format changed between OpenCode versions.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Write session marker on first tool execution for manual commit detection.
      // This allows "shiftlog store --manual" to find the session regardless of
      // how OpenCode identifies projects in its internal storage.
      if (!sessionMarkerWritten && input?.sessionID) {
        ...
        try {
          ...
          writeFileSync(dataDir + "/shiftlog-last-session.json", markerData, "utf8");
        } catch (e) {
