...
//
// On every tool execution we also write a lightweight session state file
// (.opencode/shiftlog-session.json) so that shiftlog store --manual can
// discover the active session without having to query the SQLite database.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      // Always persist the current session state so that shiftlog store --manual
      // can discover the session for post-commit note creation without relying on
      // the SQLite database (which may use a schema or query not available in all
      // environments).
      if (input.sessionID) {
        try {
          const { writeFileSync } = await import("fs");
          writeFileSync(directory + "/.opencode/shiftlog-session.json", JSON.stringify({
            session_id: input.sessionID,
            timestamp: new Date().toISOString(),
          }), { mode: 0o600 });
        } catch (e) {
          // Silently ignore — do not disrupt the workflow
        }
      }
