...
// and prevent opencode from completing.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Guarded by a 5-second timeout: opencode >=1.16 changed its internal
      // IPC and the old client.session.messages path may hang indefinitely.
      let transcriptData = "";
