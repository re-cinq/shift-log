...
//
// We also write a session marker file on every tool call so that
// "shiftlog store --manual" can discover the session even when the user
// makes a commit outside of OpenCode (post-commit git hook).
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // Write a session marker so 'shiftlog store --manual' can discover this session.
  const writeSessionMarker = async (sessionID) => {
    if (!sessionID || !directory) return;
    try {
      const { mkdirSync, writeFileSync } = await import("fs");
      const markerDir = directory + "/.shiftlog";
      mkdirSync(markerDir, { recursive: true });
      writeFileSync(markerDir + "/opencode-session.json", JSON.stringify({
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Track session on every tool call for manual-commit discovery
      if (input?.sessionID) {
