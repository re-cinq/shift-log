...
//
// On the first tool call, the session ID is written to .opencode/.shiftlog-session-id
// so that 'shiftlog store --manual' can discover the session without querying SQLite
// by project_id (which may vary across opencode versions).
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Write session ID to a marker file on the first tool call.
      // This lets 'shiftlog store --manual' find the session without
      // needing to query SQLite by project_id.
      if (!sessionMarkerWritten && input.sessionID) {
        ...
        try {
          ...
          writeFileSync(
            join(directory, ".opencode", ".shiftlog-session-id"),
            input.sessionID,
            "utf8"
          );
        } catch (e) {
          // Silently ignore — session discovery will fall back to SQLite
        }
      }
