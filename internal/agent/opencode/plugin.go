...
//
// A 5-second timeout guards the client.session.messages() call to prevent
// it from blocking opencode's process if the SDK API is unavailable or slow.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API with a 5-second timeout
      // to prevent blocking opencode if the API is unavailable or changed.
      let transcriptData = "";
      if (client && pending.sessionID) {
        try {
          ...
          // client API unavailable, timed out, or API changed — fall back to data_dir
        }
      }
