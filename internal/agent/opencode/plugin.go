...
//
// The client.session.messages SDK call is wrapped in Promise.race with a short
// timeout so that an API shape change in a newer opencode release cannot cause
// the hook to hang and block the entire opencode process.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // withTimeout races a promise against a timeout so that a hanging SDK call
  // cannot block the tool.execute.after hook indefinitely.
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API (3 s timeout guards against
      // API signature changes that would leave the promise pending forever).
      let transcriptData = "";
      if (client && pending.sessionID) {
        try {
          ...
          // Fall back to data_dir approach below (shiftlog store will query SQLite).
        }
      }
