...
// On the first tool call we also write a session marker file so the post-commit
// git hook can discover the active session for manual commits.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  const writeSessionMarker = async (sessionID) => {
    if (sessionMarkerWritten || !sessionID) return;
    sessionMarkerWritten = true;
    try {
      const { mkdirSync, writeFileSync } = await import("fs");
      const { join } = await import("path");
      const markerDir = join(directory, ".shiftlog");
      mkdirSync(markerDir, { recursive: true });
      writeFileSync(join(markerDir, "opencode-session.json"), JSON.stringify({
        session_id: sessionID,
        project_dir: directory,
        started_at: new Date().toISOString(),
      }));
    } catch (e) {
      // Silently ignore — do not disrupt the user's workflow
    }
  };

  return {
    "tool.execute.before": async (input, output) => {
      // Write session marker on first tool call so the post-commit hook
      // can discover this session for manual commits.
