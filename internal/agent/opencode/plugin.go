...
//
// The client.session.messages API changed in OpenCode v1.17 and may return a
// non-resolving Promise. A 5-second race timeout prevents the hook from hanging.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Fetch messages via the SDK client API with a timeout guard.
      // The client.session.messages API changed in OpenCode v1.17 and may not
      // resolve in all versions; the race prevents the hook from hanging.
      let transcriptData = "";
      if (client && pending.sessionID) {
        try {
          const timeout = new Promise((_, reject) =>
            setTimeout(() => reject(new Error("client API timeout")), 5000)
          );
