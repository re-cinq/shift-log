...
//
// Compatibility: supports OpenCode v1.17+ (local SQLite at .opencode/opencode.db,
// new messages/sessions schema) and v1.2–v1.16 (global SQLite, message/session schema).
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API (with 5s timeout to avoid hangs)
      let transcriptData = "";
      if (client && pending.sessionID) {
        try {
          ...
          // Fall back to local SQLite or data_dir approach below
        }
      }

      // If no transcript from client API, try local SQLite (OpenCode v1.17+)
      if (!transcriptData && pending.sessionID) {
        try {
          const { execSync: sqliteExec } = await import("child_process");
          const localDB = directory + "/.opencode/opencode.db";
          const sid = pending.sessionID.replace(/'/g, "''");
          // Try new schema (v1.17+): messages table with parts column
          try {
            const result = sqliteExec(
              "sqlite3 \"" + localDB + "\" \"SELECT json_group_array(json_object('id', id, 'role', role, 'parts', json(parts), 'created_at', created_at)) FROM messages WHERE session_id='" + sid + "' ORDER BY created_at;\"",
              { timeout: 5000 }
            );
            const data = result.toString().trim();
            if (data && data !== "[null]" && data !== "[]") {
              transcriptData = data;
            }
          } catch (e1) {
            // Try old local schema (v1.2+): message table with data column
            try {
              const result = sqliteExec(
                "sqlite3 \"" + localDB + "\" \"SELECT json_group_array(json_patch(data, json_object('id', id))) FROM message WHERE session_id='" + sid + "' ORDER BY time_created;\"",
                { timeout: 5000 }
              );
              const data = result.toString().trim();
              if (data && data !== "[null]" && data !== "[]") {
