...
//
// On every tool.execute.after we also write .shiftlog/active-session.json so that
// a manual git commit made after the session ends can still discover the session.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      // Write session marker so a manual commit made after the session can find it.
      if (input.sessionID && directory) {
        try {
          ...
          if (client && input.sessionID) {
            try {
              ...
              if (msgs && Array.isArray(msgs)) {
                transcriptJSON = JSON.stringify(msgs.map(m => ({
                  role: m.role || "",
                  id: m.id || "",
                  content: m.content || "",
                  time: m.time || {},
          ...
          if (transcriptJSON) marker.transcript_data = transcriptJSON;
          writeFileSync(join(shiftlogDir, "active-session.json"), JSON.stringify(marker), "utf8");
        } catch (_e) {}
      }
