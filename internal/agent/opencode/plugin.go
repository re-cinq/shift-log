...
//
// The plugin also writes a session cache file (.shiftlog/opencode_session.json)
// on every tool execution so that manual commits can discover the active session.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // Write session metadata to .shiftlog/opencode_session.json so that
  // 'shiftlog store --manual' can discover this session for manual commits.
  const writeSessionCache = async (sessionID) => {
    if (!sessionID) return;
    try {
      ...
      writeFileSync(
        join(cacheDir, "opencode_session.json"),
        JSON.stringify({
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Cache session ID for manual commit discovery
      await writeSessionCache(input?.sessionID);
