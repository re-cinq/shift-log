...
//
// We use --manual mode so shiftlog discovers the session itself from the
// project-local .opencode/opencode.db rather than relying on the SDK client
// API (client.session.messages), which can block indefinitely in some versions.
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      try {
        ...
        execSync("shiftlog store --manual --agent=opencode", {
          cwd: directory,
          timeout: 30000,
