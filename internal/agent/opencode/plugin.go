...
//
// We also write .shiftlog/opencode-session.json on every tool call so that
// manual commits (not made through opencode) can discover the active session.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Record the active session on every tool call so that manual commits
      // (not made through opencode) can discover the session via
      // .shiftlog/opencode-session.json without relying on SQLite schema.
