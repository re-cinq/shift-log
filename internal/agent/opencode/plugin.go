...
//
// client.session.messages() can hang in some opencode versions, so we wrap it
// with a 5-second timeout using Promise.race to prevent the hook from blocking.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Try to fetch messages via the SDK client API with a timeout to
      // prevent hanging if the API is unresponsive in this opencode version.
      let transcriptData = "";
