...
//
// The client.session.messages() call is wrapped in Promise.race with a timeout
// to prevent blocking when the OpenCode client API changes across versions.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Use Promise.race with a timeout to avoid blocking if the client API
      // changed in newer OpenCode versions and the call never resolves.
      let transcriptData = "";
