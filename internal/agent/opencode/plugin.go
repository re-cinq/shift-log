...
//
// The client.session.messages() SDK call is wrapped in Promise.race with a 5-second
// timeout because OpenCode 1.17+ changed the underlying API and the call can hang
// indefinitely when invoked with the legacy { path: { id } } signature.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Wrap in a 5-second timeout: OpenCode 1.17+ changed the underlying HTTP
      // endpoint and the legacy { path: { id } } call can return a Promise that
      // never settles, which would stall the entire hook and prevent opencode
      // from exiting.
      let transcriptData = "";
