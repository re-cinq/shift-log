...
//
// On every tool.execute.after call we also write a session snapshot to
// .shiftlog/opencode_session.json. This allows the post-commit git hook
// (shiftlog store --manual) to discover the active session without depending
// on the SQLite database schema, which changes across opencode versions.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  const writeSessionSnapshot = async (sessionID) => {
    if (!sessionID) return;
    try {
      const { mkdirSync, writeFileSync } = await import("fs");
      const snapshotDir = directory + "/.shiftlog";
      ...
      if (client) {
        try {
          ...
          // Client API may differ across opencode versions; fall through with empty messages
        }
      }

      writeFileSync(
        snapshotDir + "/opencode_session.json",
        JSON.stringify({
      ...
      // Silently ignore snapshot errors to not disrupt workflow
    }
  };
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      // Write session snapshot so the post-commit git hook can find this session.
      // Done on every tool call (not just git commits) so manual commits made after
      // the opencode session also get annotated.
      await writeSessionSnapshot(input.sessionID);
