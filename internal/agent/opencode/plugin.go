...
//
// On first tool execution the plugin also writes .shiftlog/.opencode_session with the
// session ID and data directory. This lets 'shiftlog store --manual' discover the session
// even without knowing OpenCode's internal project_id format.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // Write session info to .shiftlog/.opencode_session on first tool execution.
  // This enables 'shiftlog store --manual' to discover the session without
  // depending on OpenCode's internal project_id format or sqlite3 schema.
  const persistSession = async (sessionID) => {
    if (sessionPersisted || !sessionID) return;
    ...
    try {
      ...
      writeFileSync(join(shiftlogDir, ".opencode_session"), JSON.stringify({
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Persist session info on first tool execution for manual commit discovery
      await persistSession(input.sessionID);
