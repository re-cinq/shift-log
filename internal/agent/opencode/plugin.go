...
//
// Note: client.session.messages() is wrapped with a timeout because opencode 1.17.x
// cannot handle re-entrant SDK calls from within a plugin hook (the event loop is
// busy waiting for the hook to complete, so the SDK call never resolves).
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Use Promise.race with a short timeout: opencode 1.17.x cannot handle
      // re-entrant SDK calls from within a plugin hook, so without a timeout
      // this await would block indefinitely and hang the entire tool execution.
      let transcriptData = "";
