...
//
// On the first tool execution, the plugin also writes a session snapshot to
// .shiftlog/opencode-session.json so that "shiftlog store --manual" can
// discover the session without relying on the SQLite project_id lookup.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  let sessionSnapshotWritten = false;

  const writeSessionSnapshot = async (sessionID) => {
    if (sessionSnapshotWritten || !sessionID || !directory) return;
    sessionSnapshotWritten = true;
    try {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({ session_id: sessionID, data_dir: dataDir, project_dir: directory }),
        { encoding: "utf8" }
      );
    } catch (e) {
      sessionSnapshotWritten = false;
    }
  };

  return {
    "tool.execute.before": async (input, output) => {
      // Write session snapshot on first tool execution for --manual mode support.
