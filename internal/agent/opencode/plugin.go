...
//
// We also write a .opencode/.current-session tracking file on every tool call
// so that shiftlog store --manual can discover the session ID for manual commits,
// regardless of OpenCode's internal storage schema version.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // Compute data directory once at plugin initialization
  ...
  return {
    "tool.execute.before": async (input, output) => {
      // Write session tracking file so shiftlog store --manual can find
      // the active session for manual commits, without querying the DB.
      if (input?.sessionID) {
        try {
          const { writeFileSync, mkdirSync } = await import("fs");
          mkdirSync(directory + "/.opencode", { recursive: true });
          writeFileSync(directory + "/.opencode/.current-session", JSON.stringify({
