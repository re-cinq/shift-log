...
//
// On every tool execution we also write .shiftlog/opencode-session.json so that
// shiftlog store --manual (post-commit hook) can discover the active session
// without needing the sqlite3 CLI or OpenCode's internal storage.
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  const saveSessionMarker = async (sessionID, transcriptData) => {
    if (!sessionID) return;
    try {
      ...
      if (transcriptData) {
        marker.transcript_data = transcriptData;
      }
      fs.writeFileSync(path.join(shiftlogDir, "opencode-session.json"), JSON.stringify(marker));
    } catch (e) {
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // Always save session marker so shiftlog store --manual can find this session
      await saveSessionMarker(input?.sessionID, transcriptData);
