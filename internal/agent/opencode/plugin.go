...
// We normalise both camelCase and SCREAMING_CAPS variants of field names to support
// opencode v1.15+ which changed some property names (callID→callId, sessionID→sessionId).
...
export const ShiftlogPlugin = async ({ directory, client }) => {
  ...
  // Normalise field names across opencode versions (callID vs callId, etc.)
  const callID = (input) => input?.callID ?? input?.callId ?? input?.id ?? "";
  const sessionID = (input) => input?.sessionID ?? input?.sessionId ?? "";
  const toolName = (input) => input?.tool ?? input?.toolName ?? input?.name ?? "";
  const getCommand = (output) =>
    output?.args?.command ??
    output?.args?.cmd ??
  ...
  return {
    "tool.execute.before": async (input, output) => {
      const command = getCommand(output);
      if (command.includes("git commit") || command.includes("git-commit")) {
        pendingCommits.set(callID(input), {
          command,
          tool: toolName(input),
    ...
    "tool.execute.after": async (input, output) => {
      const pending = pendingCommits.get(callID(input));
      if (!pending) return;
      pendingCommits.delete(callID(input));
