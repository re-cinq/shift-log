...
//
// The command string is only available in the "before" hook (via output.args or input.args),
// so we capture it there and act on it in the "after" hook, matching by callID.
//
// Note: opencode 1.17+ requires `export default` for plugin discovery.
// We export both the named `ShiftlogPlugin` export (for backward compat) and
// `export default` so the plugin is recognized by all versions.
...
const _shiftlogFactory = async ({ directory, client }) => {
...
// Export both named and default for compatibility with all opencode versions.
// opencode 1.17+ requires export default for plugin discovery.
export const ShiftlogPlugin = _shiftlogFactory;
export default _shiftlogFactory;
`
