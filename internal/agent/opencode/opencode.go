...
// Handles schema differences between OpenCode <1.15 (time_updated/time_created columns)
// and OpenCode ≥1.15 (renamed to time, id embedded in data).
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// OpenCode <1.15 uses time_updated; ≥1.15 renamed it to time; fall back to rowid.
	sessionID := ""
	...
	// OpenCode <1.15 stores ISO strings; ≥1.15 stores Unix milliseconds.
	for _, q := range []string{
		...
		if !parsed {
			// Try Unix milliseconds (OpenCode ≥1.15)
	...
	// OpenCode <1.15: separate id column, ordered by time_created.
	// OpenCode ≥1.15: id embedded in data, time column renamed to time.
	var transcriptData []byte
	for _, q := range []string{
