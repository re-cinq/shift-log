...
// sqliteQueryOne runs the given SQLite queries in order, returning the trimmed
// output of the first query that succeeds and produces non-empty output.
...
// parseFlexibleTime parses a time value in various formats including
// ISO 8601 variants, SQLite datetime strings, and Unix timestamps
...
func parseFlexibleTime(s string) (time.Time, error) {
	...
	// Try Unix timestamp integer (seconds or milliseconds)
	if n, err := strconv.ParseInt(s, 10, 64); err == nil {
		if n > 1_000_000_000_000 { // likely milliseconds (> year 2001 in ms)
...
//   - Pre-1.16: separate time_created / time_updated columns
//   - 1.16+:    a single JSON 'time' column with "created"/"updated" keys
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Try time-ordered queries first (best result), then unordered as fallback.
	...
	// Try various column forms to support different schema versions.
	...
	if timeStr != "" {
		if t, err := parseFlexibleTime(timeStr); err == nil {
	...
	// Try time-ordered first, fall back to unordered for different schema versions.
	msgOutput := sqliteQueryOne(dbPath,
	...
	// sqlite3 returns "[null]" when no rows match aggregate with NULL data
