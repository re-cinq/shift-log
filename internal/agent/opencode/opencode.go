...
func parseOpenCodeEntry(raw map[string]json.RawMessage, fullData []byte) agent.TranscriptEntry {
	...
	// Parse timestamp — try nested time.created (legacy) then flat created_at (current schema).
	...
	if entry.Timestamp == "" {
		if tsRaw, ok := raw["created_at"]; ok {
			entry.Timestamp = strings.Trim(string(tsRaw), `"`)
		}
	}
...
// Handles both the current "parts" array format and the legacy "content" field.
func parseOpenCodeMessage(raw map[string]json.RawMessage, msgType agent.MessageType) *agent.Message {
	...
	// Try "parts" array (current OpenCode schema: typed part wrappers).
