...
// It checks locations in priority order:
//  1. Project-local SQLite (OpenCode 1.15+: .opencode/opencode.db, new schema)
//  2. Legacy flat file storage (pre-v1.2 OpenCode)
//  3. Global XDG SQLite (OpenCode v1.2-1.14: ~/.local/share/opencode/opencode.db)
func (a *Agent) DiscoverSession(projectPath string) (*agent.SessionInfo, error) {
	// Try project-local SQLite first (OpenCode 1.15+)
	localDB := filepath.Join(projectPath, ".opencode", "opencode.db")
	if _, err := os.Stat(localDB); err == nil {
		if session, err := discoverFromLocalSQLite(localDB, projectPath); session != nil || err != nil {
			return session, err
		}
	}
	...
	// Fall back to global XDG SQLite (OpenCode v1.2-1.14)
...
// Handles the OpenCode 1.15+ schema: sessions/messages (plural), updated_at/created_at,
// parts column, no project_id column.
func discoverFromLocalSQLite(dbPath, projectPath string) (*agent.SessionInfo, error) {
	if _, err := exec.LookPath("sqlite3"); err != nil {
		return nil, nil
	}

	// New schema: sessions table (plural), updated_at ordering, no project_id filter
	...
	// Get messages: new schema uses messages (plural) table with parts column
...
// discoverFromSQLite queries the OpenCode global XDG SQLite database.
// Handles the OpenCode v1.2-1.14 schema: session/message (singular), time_updated/time_created,
// data column, project_id filter.
...
func parseOpenCodeMessage(raw map[string]json.RawMessage, msgType agent.MessageType) *agent.Message {
	...
	// Try "parts" field (OpenCode 1.15+ format: typed parts array)
