...
// sqliteQuerySession returns the ID of the most recent session for the given project.
// Tries ordering by time_updated first (pre-v1.15 schema), then falls back to rowid
// for schemas where the time column was renamed or restructured.
...
// or if recency cannot be determined (we err on the side of inclusion).
// Tries time_updated column first (pre-v1.15), then json_extract(time, '$.updated') for
// schemas where the time data is stored as a nested JSON object.
...
// sqliteQueryMessages returns the transcript JSON array for the given session.
// Tries time_created ordering first (pre-v1.15 schema), then falls back to rowid.
// As a last resort omits the id column injection via json_patch.
