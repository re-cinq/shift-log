...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Try multiple ORDER BY variants to handle different opencode schema versions.
	...
	// Try multiple time column names for different opencode schema versions.
	for _, col := range []string{"time_updated", "updated_at"} {
		...
		// If time is unparseable (e.g., Unix ms integer), proceed — better to try than skip
		break
	}

	// Get messages for this session as a JSON array.
	// Try multiple query variants to handle different opencode schema versions.
	// If all queries fail, fall back to an empty transcript so the git note is still created.
	transcriptData := []byte("[]")
