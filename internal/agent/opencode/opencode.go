...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// OpenCode ≥ v1.15 stores time as a JSON blob {"created": ms, "updated": ms}
	// in a column named "time"; older SQLite-enabled releases used a separate
	// "time_updated" column. Fall back to rowid ordering as a last resort.
	sessionID := ""
	...
	// Same schema-version fallback as above.
	for _, q := range []string{
		...
		// Try as Unix milliseconds (opencode ≥ v1.15 format)
		...
		// Try standard time string formats (older schema)
		parsed := false
