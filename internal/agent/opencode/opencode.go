...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// dependency on specific timestamp column names that vary across OpenCode
	// versions (e.g. time_updated in older versions vs a JSON time column in v1.15+).
	...
	// Try multiple column name patterns used across different OpenCode versions:
	//   - time_updated (pre-v1.15)
	//   - time JSON object with $.updated (v1.15+)
	//   - time JSON object with $.created (v1.15+ fallback)
	//   - time_created (older fallback)
	timeQueries := []string{
	...
	for _, q := range timeQueries {
		...
		break // used first column that returned a value; unparseable timestamps proceed
	}
	// If no time column returned a value, proceed anyway — better to try than skip.

	// Get messages for this session as a JSON array.
	// Use rowid for ordering instead of time_created to avoid column name drift.
