...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Use rowid for ordering — time column names vary across OpenCode versions.
	...
	// Try common column names — the schema varies across OpenCode versions.
	for _, col := range []string{"time_updated", "updated_at", "time_created", "created_at"} {
		...
		tcmd := exec.Command("sqlite3", dbPath, timeQuery)
		timeOutput, terr := tcmd.Output()
		if terr != nil {
			continue
		}
		...
		break // successfully queried a time column; proceed
	}
