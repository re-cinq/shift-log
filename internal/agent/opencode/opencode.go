...
// It tries local SQLite (v1.17+), then flat files (pre-v1.2), then global SQLite (v1.2-v1.16).
func (a *Agent) DiscoverSession(projectPath string) (*agent.SessionInfo, error) {
	// Try local SQLite first (OpenCode v1.17+: .opencode/opencode.db)
...
// OpenCode v1.17+ stores its database at {projectPath}/.opencode/opencode.db
// using sessions/messages tables (plural) without a project_id column.
func discoverFromLocalSQLite(projectPath string) (*agent.SessionInfo, error) {
	...
	// v1.17+ schema: sessions table with updated_at (Unix ms), no project_id
	...
	if err == nil {
		...
		sessionTime := parseUnixTimestamp(timeStr)
		if !sessionTime.IsZero() && time.Since(sessionTime) > agent.RecentSessionTimeout {
			return nil, nil
		}
	}

	// Get messages as JSON array with role and parts (v1.17+ schema)
...
// parseUnixTimestamp parses a Unix timestamp string (seconds or milliseconds).
...
func parseOpenCodeMessage(raw map[string]json.RawMessage, msgType agent.MessageType) *agent.Message {
	...
	// Try "parts" field (opencode v1.17+ format: typed array of message parts)
