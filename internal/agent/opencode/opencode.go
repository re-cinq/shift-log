...
// It tries project-local SQLite first (v1.15+), then flat file storage (pre-v1.2),
// then falls back to the global SQLite (v1.2-v1.14).
...
func discoverFromLocalSQLite(projectPath string) (*agent.SessionInfo, error) {
	...
	// Get the most recent session with its timestamp
	...
	// Check recency using updated_at (stored as Unix milliseconds)
...
func parseOpenCodeEntry(raw map[string]json.RawMessage, fullData []byte) agent.TranscriptEntry {
	...
	// Parse timestamp — "time" object (legacy) or "created_at" integer (v1.15+)
	...
	if entry.Timestamp == "" {
		if createdAtRaw, ok := raw["created_at"]; ok {
			var ts int64
			if err := json.Unmarshal(createdAtRaw, &ts); err == nil && ts > 0 {
				if ts > 1e12 {
					entry.Timestamp = time.Unix(ts/1000, 0).Format(time.RFC3339)
				} else {
					entry.Timestamp = time.Unix(ts, 0).Format(time.RFC3339)
				}
			}
		}
	}
