...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Use ROWID for ordering to avoid depending on specific column names
	// that changed between opencode versions (pre-v1.17 used time_updated;
	// v1.17+ stores time as a JSON column).
	...
	// Use ROWID for ordering to avoid depending on time_created column
	// which may not exist in newer opencode versions.
...
// or true if the session time cannot be determined.
// It tries multiple column/expression variants to handle opencode schema changes:
//   - pre-v1.17: standalone time_updated column
//   - v1.17+: time column storing JSON {"created":"...","updated":"..."}
func isSessionRecent(dbPath, sessionID string) bool {
	...
	for _, q := range candidates {
		...
		// Got a value but couldn't parse its format — proceed optimistically
		return true
	}
	// Couldn't retrieve any time value — proceed optimistically
	return true
}
