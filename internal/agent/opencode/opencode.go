...
// OpenCode v1.2+ stores sessions in SQLite with a `time` column containing a JSON
// object {"created":"...","updated":"..."} rather than separate time_updated/time_created columns.
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Try new schema (time as JSON blob with $.updated) first, then legacy (time_updated column).
	...
	// Try both new schema (JSON time blob) and legacy (time_updated column).
	for _, q := range []string{
	...
	// Try new schema ordering (json_extract), then legacy (time_created), then unordered.
	var transcriptData []byte
	for _, q := range []string{
