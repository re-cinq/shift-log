...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Try multiple ORDER BY strategies for schema compatibility across opencode versions:
	// pre-1.17 used time_updated column; newer versions may use rowid or different names.
	sessionID := ""
	for _, orderBy := range []string{"time_updated DESC", "rowid DESC"} {
		q := fmt.Sprintf(`SELECT id FROM session WHERE project_id='%s' ORDER BY %s LIMIT 1;`, projectID, orderBy)
	...
	// Try multiple column/expression forms for schema compatibility.
	for _, col := range []string{
		...
		if parseErr == nil {
			if time.Since(parsed) > agent.RecentSessionTimeout {
				return nil, nil
			}
			break
		}
	...
	// Try multiple ORDER BY strategies for schema compatibility.
	var transcriptData []byte
