...
// storage (pre-v1.2), then the global XDG data directory SQLite (v1.2-v1.16).
func (a *Agent) DiscoverSession(projectPath string) (*agent.SessionInfo, error) {
	// Try project-local SQLite first (OpenCode v1.17+: .opencode/opencode.db)
	localDBPath := filepath.Join(projectPath, ".opencode", "opencode.db")
	if _, err := os.Stat(localDBPath); err == nil {
		if session, err := discoverFromLocalSQLite(localDBPath, projectPath); err == nil && session != nil {
			return session, nil
		}
	}
...
// The schema uses "sessions"/"messages" (plural) tables with integer Unix-ms timestamps.
func discoverFromLocalSQLite(dbPath, projectPath string) (*agent.SessionInfo, error) {
	if _, err := exec.LookPath("sqlite3"); err != nil {
		return nil, nil
	}

	// v1.17+ uses "sessions" (plural) with "updated_at" stored as integer Unix milliseconds
	...
	// v1.17+ uses "messages" (plural) with a "parts" JSON column and "created_at" integer ms.
