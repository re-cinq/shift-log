...
// sqliteQueryFirst runs sqlite3 queries in order and returns the trimmed stdout
// of the first query that succeeds with non-empty output.
...
// It tries multiple SQL expressions to handle different schema versions:
//   - Older schema: explicit time_updated / time_created columns
//   - Newer schema (v1.15+): a JSON "time" column with {created, updated} fields
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Try both column-name styles for the timestamp.
	...
	msgJSON := sqliteQueryFirst(dbPath, []string{
