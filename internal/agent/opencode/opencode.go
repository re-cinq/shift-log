...
// opencode v1.15+ stores timestamps in a JSON `time` column ({created, updated});
// older versions used separate `time_updated`/`time_created` columns.
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Use rowid DESC as a schema-agnostic ordering fallback; it reflects insertion order.
	sessionID := ""
	...
	// Try the v1.15+ JSON time column first, then the legacy time_updated column.
	timeQueries := []string{
	...
	for _, q := range timeQueries {
		...
		// Successfully checked (or couldn't parse — proceed anyway)
		break
	}

	// Get messages for this session as a JSON array.
	// Order by rowid for schema-agnostic insertion order; fall back to time_created.
	msgQueries := []string{
