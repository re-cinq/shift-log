...
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Use rowid DESC for ordering — more portable than time_updated, which was
	// moved into a JSON 'time' column in OpenCode v1.17+.
	...
	// Try multiple column/JSON-path patterns to handle schema changes across versions.
	if !isSessionRecent(dbPath, sessionID) {
		return nil, nil
	}

	// Get messages for this session as a JSON array.
	// Use rowid for ordering — more portable than time_created, which was
	// moved into a JSON 'time' column in OpenCode v1.17+.
...
// It tries multiple column and JSON-path patterns to handle schema changes across versions.
func isSessionRecent(dbPath, sessionID string) bool {
	// Try different ways to read the update time across OpenCode versions.
	// v1.17+: time stored as JSON object in 'time' column: {"created":..., "updated":...}
	// older:  separate time_updated column
	queries := []string{
