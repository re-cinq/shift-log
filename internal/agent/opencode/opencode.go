...
// (e.g., v1.2 used time_created/time_updated columns; v1.15+ uses a JSON time column).
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Try ORDER BY variants for compatibility across OpenCode schema versions.
	...
	// Try multiple variants for compatibility across OpenCode schema versions:
	//   - Pre-v1.15: separate id column, time_created for ordering
	//   - v1.15+:    time stored as JSON object in 'time' column; id may be in data
	transcriptStr := sqliteQueryFirst(dbPath,
	...
	if transcriptStr == "" || transcriptStr == "[null]" || transcriptStr == "[]" {
		return nil, nil
	}
...
// sqliteQueryFirst runs sqlite3 with each query in order, returning the first
// non-empty result. Used to handle schema differences across OpenCode versions.
