...
//   - Pre-v1.16: separate time_updated / time_created columns
//   - v1.16+: a single time JSON column ({"created":"...","updated":"..."})
func discoverFromSQLite(dataDir, projectID, projectPath string) (*agent.SessionInfo, error) {
	...
	// Try different ORDER BY expressions to handle schema changes across versions.
	sessionID := sqliteQueryFirst(dbPath, []string{
		// pre-v1.16: time_updated column
		fmt.Sprintf(`SELECT id FROM session WHERE project_id='%s' ORDER BY time_updated DESC LIMIT 1;`, projectID),
		// v1.16+: time stored as JSON {"created":"...","updated":"..."}
	...
	// Try different strategies to handle schema and ordering changes.
	msgQueries := []string{
		// pre-v1.16: id column + time_created ordering
		...
		// v1.16+: time JSON column ordering
	...
	for _, q := range msgQueries {
		...
		transcriptData = []byte(d)
		break
	}

	if transcriptData == nil {
		// Session exists but no messages yet — store with empty transcript.
		transcriptData = []byte("[]")
	}
...
// sqliteQueryFirst runs each query in order against dbPath and returns the first
// non-empty single-column result. Returns "" if all queries fail or produce no output.
